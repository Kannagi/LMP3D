...
{
	float sinAngle = AURAE_sinf(angle);
	float cosAngle = AURAE_cosf(angle);
...
void AURAE_MatrixMultiplyFPU(float* dest,float* src1,float* src2)
{
	int i;
...
void AURAE_MatrixMultiply2(float* dest,float* src1,float* src2)
{
	int i;
	float x,y,z,w;
...
Vector3 AURAE_TransformPerspectiveFPU(float* matrix,Vector3 vec)
{
	register float x,y,z,w;
...
void AURAE_MatrixView(AURAE_Model *model,float * matrixFinal)
{
	...
	if(model->rotate.y != 0)
	{
		AURAE_MatrixRotateY(rotateMatrix, model->rotate.y);

		if(model->rotate.x != 0)
		{
			AURAE_MatrixRotateX(rotateMatrix2, model->rotate.x);
			AURAE_MatrixMultiply(rotateMatrix, rotateMatrix2,rotateMatrix);
		}

		if(model->rotate.z != 0)
		{
			AURAE_MatrixRotateZ(rotateMatrix2, model->rotate.z);
			AURAE_MatrixMultiply(rotateMatrix, rotateMatrix2,rotateMatrix);
		}
	}else
	{
		if(model->rotate.x != 0)
		{
			AURAE_MatrixRotateX(rotateMatrix, model->rotate.x);
		...
		{
			if(model->rotate.z != 0)
				AURAE_MatrixRotateZ(rotateMatrix, model->rotate.z);
			else rotate = 0;
		}
	}

	//T+S
	AURAE_MatrixTS(matrixFinal, model->position.x, model->position.y,model->position.z, model->scale.x, model->scale.y, model->scale.z);

	if(rotate == 1)
	{
		//T+S+R
		AURAE_MatrixMultiply(matrixFinal,rotateMatrix, matrixFinal);
	}
}
