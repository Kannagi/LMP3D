...
Vector3 AURAE_MatrixVecTransformPerspectiveFPU(float* matrix,Vector3 vec);
float *AURAE_MatrixProjection_Get();

float *AURAE_Camera_LookAt_Get();
void MNS_draw_skeleton_anim(AURAE_Model *model,int j)
{
...
void MNS_draw_skeleton(AURAE_Model *model)
{
	if(model->nbones == 0) return;
...
void AURAE_Model_Draw_Test(AURAE_Model *model,float *matrixView,float *buffer_vertex)
{
	int i;
	float matrix[16];
	glMatrixMode(GL_PROJECTION);
	AURAE_MatrixIdentity(matrix);
	...
	AURAE_MatrixMultiply(matrix,matrixView,matrixProjection);
	...
	for(i = 0;i < model->nv*3;i+=3)
	{
		...
		vec = AURAE_TransformPerspectiveFPU(matrix,vec);
...
void AURAE_Model_Draw_Test2(AURAE_Model *model,float *buffer_vertex)
{
	int i;
	memcpy(buffer_vertex,model->v,model->nv*3*4);
...
void AURAE_Model_Draw_TestAnim(AURAE_Model *model,float *buffer_vertex)
{
	int i,l,k;
	memcpy(buffer_vertex,model->v,model->nv*3*4);
...
*/
	...
	*/
	...
	for(l = 0;l < model->nbones;l++)
	{
		AURAE_MatrixIdentity(rotateMatrix);
		k = l*16;
		...
		//AURAE_MatrixTS(rotateMatrix2,x,y,z, 1, 1,1);

		//AURAE_MatrixRotateX(rotateMatrix, model->anim.skeleton[l].matrix[0+3]);
		//AURAE_MatrixMultiply(rotateMatrix2, rotateMatrix,rotateMatrix2);
		//AURAE_MatrixMultiply(rotateMatrix2,model->anim.skeleton[l].matrix,rotateMatrix2);

		//rotateMatrix2[3+0] = -rotateMatrix2[3+0];
		//rotateMatrix2[3+8] = -100;
		//rotateMatrix2[3+4] = -rotateMatrix2[3+4];

		//AURAE_MatrixRotate(rotateMatrix, (float)l/20.0);
		//AURAE_MatrixMultiply(rotateMatrix2, rotateMatrix,rotateMatrix2);
		...
		AURAE_MatrixRotateZ(rotateMatrix, rz);

		AURAE_MatrixRotateX(rotateMatrix2, rx);
		AURAE_MatrixMultiply(rotateMatrix, rotateMatrix2,rotateMatrix);

		AURAE_MatrixRotateY(rotateMatrix2, ry);
		AURAE_MatrixMultiply(rotateMatrix, rotateMatrix2,rotateMatrix);
		...
		for(i = model->bones[l];i < model->bones[l+1];i++)
		{
			...
			AURAE_MatrixVec3MultiplyFPU(&buffer_vertex[j+0],rotateMatrix);
...
/*
		...
		vec = AURAE_MatrixVecMultiplyFPU(model->anim.skeleton[k].matrix,vec);
...
*/
	...
	for(i = model->bones[l];i < model->bones[l+1];i++)
	{
		...
		AURAE_MatrixRotateY(rotateMatrix, model->anim.skeleton[0].r.y);

		AURAE_MatrixRotateX(rotateMatrix2, model->anim.skeleton[0].r.x);
		AURAE_MatrixMultiply(rotateMatrix, rotateMatrix2,rotateMatrix);

		AURAE_MatrixRotateZ(rotateMatrix2, model->anim.skeleton[0].r.z);
		AURAE_MatrixMultiply(rotateMatrix, rotateMatrix2,rotateMatrix);


		vec = AURAE_MatrixVecMultiplyFPU(rotateMatrix,vec);
...
/*
	if(model->nbones != 0)
	{
		AURAE_Model_Draw_TestAnim(model,buffer_vertex);
		vertex = buffer_vertex;
	}*/

	int i,end,begin,sizeindex = GL_UNSIGNED_SHORT;
	if(model->flag & AURAE_MODEL_INDEX_U32) sizeindex = GL_UNSIGNED_INT;

	glEnableClientState( GL_VERTEX_ARRAY );
	glVertexPointer(3,GL_FLOAT,0,vertex);

	if(model->flag & AURAE_MODEL_TEXTCOORD)
	{
	...
	if(model->flag & AURAE_MODEL_NORMAL)
	{
	...
	//model->ntexture = 0;
	if((model->flag & AURAE_MODEL_TEXTUREDISABLE) || model->ntexture == 0)
	{
		if(model->flag & AURAE_MODEL_TEXTUREDISABLE)
			glBindTexture(GL_TEXTURE_2D,0);
