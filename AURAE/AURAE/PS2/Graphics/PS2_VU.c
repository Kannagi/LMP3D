...
void PS2_VU_Load(AURAE_Model *model)
{
	void *data = model->v;
...
void PS2_VU_Draw(float* matrix,AURAE_Model *model)
{
	...
	//Vector3 vec = AURAE_TransformPerspectiveFPU(matrix,AURAE_Type_Vector3(0,0,-200));
...
*/
	for(i = 0;i < model->ngroup;i+=2)
	{
		...
		if(n >= model->texture_begin[l])
			l++;
		AURAE_Texture_Setup(model->texture[l]);
		n += model->groupface[i];
