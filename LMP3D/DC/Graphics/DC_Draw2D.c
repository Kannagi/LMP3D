...
void LMP3D_Draw_Sprite(LMP3D_Texture *texture,Vector2i position,Recti *rectin,int flag)
{
	...
	*((u32*)currentBuffer+0) = 0XF0000000;
	...
	if(flag & LMP3D_FLIPH) flagw = frectw+zfrectw;
	if(flag & LMP3D_FLIPV) flagh = frecth+zfrecth;
...
void __attribute__((optimize("-O1"), noinline)) LMP3D_Draw_TileMap(LMP3D_TileMap *tilemap)
{
	int i=0,flag;
	...
	*((float*)currentBuffer+9) = z;
	...
	for(y = 0;y < ntiley;y++)
	{
		for(x = 0;x < ntilex;x++)
		{
			...
			float flagw = 0,flagh = 0;
			if(flag & LMP3D_FLIPH) flagw = tilef;
			if(flag & LMP3D_FLIPV) flagh = tilef;
